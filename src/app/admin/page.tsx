      ...
      <>
        ...
        <main className="pt-14 min-h-screen bg-white dark:bg-darkBg flex items-center justify-center">
          <div className="w-full max-w-xs mx-auto px-4 text-center">
            <p className="text-[0.6rem] tracking-[0.22em] uppercase text-faint dark:text-darkFaint mb-8 font-medium">Admin</p>
            <h1 className="text-2xl font-semibold tracking-tight text-ink dark:text-white mb-2">Enter PIN</h1>
            <p className="text-xs text-muted dark:text-darkMuted mb-10">4-digit access code</p>
            <div className="flex justify-center gap-3 mb-6">
              {digits.map((d, i) => (
                <input key={i} ref={digitRefs[i]} type="password" inputMode="numeric" maxLength={1} value={d}
        ...
        <div className="max-w-wide mx-auto px-4 sm:px-6 py-10 sm:py-16">
          ...
          <div className="flex items-start justify-between mb-8">
            <div>
              ...
              <h1 className="text-xl font-semibold tracking-tight text-ink dark:text-white">Recipe Manager</h1>
            </div>
            <button onClick={handleLogout} className="text-xs text-faint dark:text-darkFaint hover:text-muted dark:hover:text-darkMuted transition-colors mt-1">
              Lock
            </button>
          </div>
          ...
          {loading && (
            <div className="space-y-3">
              {[1, 2, 3].map((i) => <div key={i} className="h-16 bg-lift dark:bg-darkSurface rounded-xl animate-pulse" />)}
            </div>
          )}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-rule dark:border-darkBorder bg-lift dark:bg-darkSurface">
                    <th className="text-left px-5 py-3 text-[0.6rem] tracking-widest uppercase text-faint dark:text-darkFaint font-medium">Name</th>
                    <th className="text-left px-5 py-3 text-[0.6rem] tracking-widest uppercase text-faint dark:text-darkFaint font-medium hidden sm:table-cell">Category</th>
                    ...
                    <th className="text-right px-5 py-3 text-[0.6rem] tracking-widest uppercase text-faint dark:text-darkFaint font-medium">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {displayed.map((r) => (
